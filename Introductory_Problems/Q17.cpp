...
int main(){
    int q;
    cin>>q;
    vector<long long>powerof(19,1);
    for(int i=1;i<19;i++) powerof[i]=10*powerof[i-1]; //power of 18 on one based index
    while(q--){
        long long index;
        cin>>index;
        long long sofar=0;
        ...
        int numdigits;
        for(int i=1;i<=18;i++){  //calculate number of digits in the required number
